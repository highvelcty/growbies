...
from growbies.utils.types import Serial_t, SerialOrDeviceID_t
...
class DevicesEngine:
    ...
    def set_connected(self, *serials_or_ids: Serial_t):
        with self._engine.new_session() as db_session, db_session.begin():
            self._set_flag(*serials_or_ids, flag=ConnectionState.CONNECTED, value=True,
                           db_session=db_session)

    def clear_connected(self, *serials_or_ids: Serial_t):
        with self._engine.new_session() as db_session, db_session.begin():
            self._set_flag(*serials_or_ids, flag=ConnectionState.CONNECTED, value=False,
                           db_session=db_session)
