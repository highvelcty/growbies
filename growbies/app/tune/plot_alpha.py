...
# -----------------------------------------------------------------------------
# Filter math (LINEAR model)
# -----------------------------------------------------------------------------

def compute_alpha(error, alpha_min, alpha_max, alpha_threshold):

    # Linear growth instead of sqrt
    alpha = error / alpha_threshold
...
ax_alpha.set_title("AEWMA Alpha vs Error (Linear)")
...
def update(_):
    ...
    event_line_alpha.set_xdata([s_event.val, s_event.val])
    event_line_window.set_xdata([s_event.val, s_event.val])
